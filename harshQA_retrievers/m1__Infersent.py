...
class m1_Infersent(BaseEstimator):
    """
    A scikit-learn estimator for Infersent Retriever. Update the vocab with the corpus content and
    create an embedding matrix, that get transformed with a PCA.
    Then finds the most top_N similar documents of a given input document by
    taking the dot product of the vectorized input document and the new embedding matrix.
    ...
    model_path: str
        Infersent2 model .pkl 
        donwload it here : https://github.com/facebookresearch/InferSent

    w2v_path: str
        FasText or Glove weights .vec
        download it here https://github.com/facebookresearch/InferSent



    Examples
    --------
    >>> retriever = m1_Infersent(top_n=5, model_path=m_path, w2v_path=w_path)
...
class InferSent_(nn.Module):

    """
        Infersent Model, modified for word importance visualization normalized to 1, useful for query denoising and removal 
        of unnecessary words

    """

    def __init__(self, config):
        super(InferSent_, self).__init__()
        self.bsize = config['bsize']
        self.word_emb_dim = config['word_emb_dim']
        self.enc_lstm_dim = config['enc_lstm_dim']
        self.pool_type = config['pool_type']
        self.dpout_model = config['dpout_model']
        self.version = 2 if 'version' not in config else config['version']
